...
def safe_str(val):
    """Safely convert value to string and escape XML special characters."""
    if val is None:
        return ""
    return html.escape(str(val))

def generate_pdf(question_paper):
    buffer = BytesIO()
    ...
    # Process Questions
    questions = getattr(question_paper, 'questions', [])
    for idx, q in enumerate(questions, 1):
        q_elements = []
        
        # Question Header/Text
        q_num = safe_str(getattr(q, 'question_number', None) or f"Q{idx}")
        ...
        # Keep each question and its options together on the same page
